...
const skills = Object.keys(spellList)

for (let index = 0; index < skills.length; index++) {
    const skillName = skills[index];
    const skill = spellList[skillName]
    const listOfSpecies = skill.species
    if(listOfSpecies) {
        listOfSpecies.forEach(species => {
            if(species && racesList[species]) {
                if(racesList[species].links?.skills) {
                    racesList[species].links.skills.push(skillName)
                } else {
                    racesList[species].links = {}
                    racesList[species].links.skills = [skillName]
                }
            } else if (species) {
                racesList[species] = {
                    name: species,
...
const jsonString = JSON.stringify(racesList, null, 2);
